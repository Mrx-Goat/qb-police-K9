...
RegisterNetEvent('angelicxs-k9script:searchingcar', function()
	if DoesEntityExist(Dog) then
		...
		if Config.UseESX then
			...
			if not DoesEntityExist(VehicleData) then
				TriggerEvent('angelicxs-k9scipt:Notify', Config.Lang['no_vehicle'], Config.LangType['error'])
				return
			end
		...
		while not searchComplete and GetGameTimer() < timeout do
			Wait(50)
		end
